...
export class ContactsComponent implements OnInit {
  ...
  ngOnInit() {
    this.form = this.fb.group({
      ...
      message: ['', [Validators.required, Validators.pattern('[a-zA-Z0-9 ]{1,296}'), Validators.maxLength(256)]]
    });
  }
  onSubmit() {
    ...
    this.addGoogle(path, formData).toPromise()
    .then((data) => console.log(data))
    .catch((err) => console.log(err, 'sssserr'));
    if (this.form.valid) {
      this.successHandler();
    }
  }
  ...
  addGoogle(path: string, body: Object = {}): Observable<any> {
    const baseApiUrl = 'https://docs.google.com';
    return this.http
      .post(`${baseApiUrl}${path}`, body)
      .pipe(
